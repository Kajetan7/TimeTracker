const apikey = '246a66a7-4c35-4250-9620-f93f4606ef16';
...
function renderTask(taskId, title, description, status) {
    ...
    apiListOperationsForTask(taskId).then(
        function(response) {
            response.data.forEach(
                function(operation) { renderOperation(ul, status, operation.id, operation.description, operation.timeSpent); }
            );
        }
    )
    ...
    if(status == 'open') {
        ...
        descriptionInput.setAttribute('minilength', '5');
        ...
        form.addEventListener("submit", function(event) {
            ...
            apiCreateOperationForTask(taskId, descriptionInput.value).then(
                function (operation) { renderOperation(ul, status, operation.data.id, operation.data.description, operation.data.timeSpent)}
            )
        });
    }
}
